...
## Lengths are in meters, prefactors are in m/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>9.76&x<26.52,5.9e-11,2.16e-10))'
  [../]

  [./Thickness-Power-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>9.76&x<26.52,0.517,0.503))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>9.76&x<26.52,1e-2,1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Ni]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>9.76&x<26.52,3.32,3.32))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1e-10,if(x>9.76&x<26.52,6.1e-2,6.1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,1e-10,if(x>9.76&x<26.52,3.39,3.39))'
  [../]
...
## The order of the numbers goes Core_Length, Core_metal, SG_Start, SG_End, SG_metal, Hot/Cold_Leg_metal
    ...
    value = 'if(x<3.66,0,if(x>9.76&x<26.52,0.72,0.1))'
  [../]

  [./Metal-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>9.76&x<26.52,0.1,0.7))'
  [../]

  [./ND-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0.1,if(x>9.76&x<26.52,9.14e28,8.49e28))'
  [../]
[]
...
## All length and wetted perimeters should be in meters
    ...
    function = 'if(x<3.66,1384.30,if(x>9.76&x<26.52,832.5,2.31))'
  [../]
