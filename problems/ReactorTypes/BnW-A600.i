...
## Lengths are in meters, prefactors are in m/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>26.52&x<42.37,5.9e-11,2.16e-10))'
  [../]

  [./Thickness-Power-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,0.517,0.503))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>26.52&x<42.37,1e-2,1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Ni]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,3.32,3.32))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1e-10,if(x>26.52&x<42.37,6.1e-2,6.1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,1e-10,if(x>26.52&x<42.37,3.39,3.39))'
  [../]
...
## The order of the numbers goes Core_Length, Core_metal, SG_Start, SG_End, SG_metal, Hot/Cold_Leg_metal
    ...
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,0.72,0.1))'
  [../]
...
## All length and wetted perimeters should be in meters
    ...
    function = 'if(x<3.66,1704.34,if(x>26.52&x<42.37,1358.92,2.31))'
  [../]
