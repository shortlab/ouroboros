...
## Lengths are in meters, prefactors are in m/sec, powers are unitless
    ...
    value = 'if(x<3.66,2.16e-10,if(x>9.76&x<26.52,5.9e-11,2.16e-10))'
  [../]

  [./Thickness-Power-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0.503,if(x>9.76&x<26.52,0.517,0.503))'
  [../]

  [./Thickness-Offset-Function]
    type = ParsedFunction
    value = 'if(x<3.66,10e-9,if(x>9.76&x<26.52,10e-9,10e-9))'
  [../]
...
## Lengths are in meters, prefactors are in m^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1.59e-9,if(x>9.76&x<26.52,1.59e-9,1.59e-9))'
  [../]

  [./Oxide-Activation-Energy-Function-Ni]
    type = ParsedFunction
    value = 'if(x<3.66,1.84,if(x>9.76&x<26.52,1.84,1.84))'
  [../]
...
## Lengths are in meters, prefactors are in m^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,6.14e-9,if(x>9.76&x<26.52,6.14e-9,6.14e-9))'
  [../]

  [./Oxide-Activation-Energy-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,1.87,if(x>9.76&x<26.52,1.87,1.87))'
  [../]
...
## The order of the numbers goes Core_Length, Core_metal, SG_Start, SG_End, SG_metal, Hot/Cold_Leg_metal
    ...
    value = 'if(x<3.66,0.1,if(x>9.76&x<26.52,0.72,0.1))'
  [../]

  [./Metal-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,0.7,if(x>9.76&x<26.52,0.1,0.7))'
  [../]

  [./ND-Function]
    type = ParsedFunction
    value = 'if(x<3.66,8.49e28,if(x>9.76&x<26.52,9.14e28,8.49e28))'
  [../]
[]
...
## All length and wetted perimeters should be in meters
    ...
    function = 'if(x<3.66,1704.34,if(x>9.76&x<26.52,1110,2.31))'
  [../]
...
## All temperatures are in Kelvin
    ...
    function = 'if(x<3.66,565+(25*sin(x*3.14159/3.66))+34*(x/3.66),if(x>9.76&x<26.52,(599-34*((x-9.76)/16.76)),if(x>26.52,565,599)))'
  [../]
[]
