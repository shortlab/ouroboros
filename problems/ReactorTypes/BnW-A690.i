...
## Lengths are in meters, prefactors are in m/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>26.52&x<42.37,5.25e-9,2.16e-10))'
  [../]

  [./Thickness-Power-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,0.154,0.503))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>26.52&x<42.37,1.5e-3,1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Ni]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,4.28,3.32))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1e-10,if(x>26.52&x<42.37,1.51e-3,6.1e-2))'
  [../]

  [./Oxide-Activation-Energy-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,1e-10,if(x>26.52&x<42.37,4.02,3.39))'
  [../]
...
## The order of the numbers goes Core_Length, Core_metal, SG_Start, SG_End, SG_metal, Hot/Cold_Leg_metal
    ...
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,0.6,0.1))'
  [../]

  [./Metal-Function-Fe]
    type = ParsedFunction
    value = 'if(x<3.66,0,if(x>26.52&x<42.37,0.1,0.7))'
  [../]

  [./ND-Function]
    type = ParsedFunction
    value = 'if(x<3.66,0.1,if(x>26.52&x<42.37,9.14e28,8.49e28))'
  [../]
[]
...
## All length and wetted perimeters should be in meters
    ...
    function = 'if(x<3.66,1704.34,if(x>26.52&x<42.37,1417.32,2.31))'
  [../]
...
## All temperatures are in Kelvin
    ...
    function = 'if(x<3.66,565+(25*sin(x*3.14159/3.66))+34*(x/3.66),if(x>26.52&x<42.37,(599-34*((x-26.52)/15.85)),if(x>42.37,565,599)))'
  [../]
[]
