...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1,if(x>9.76&x<31.1,1.5e-3,1e-3))'
  [../]

  [./Oxide-Activation-Energy-Function-Ni]
    type = ParsedFunction
#    value = 'if(x<3.66,0,if(x>9.76&x<31.1,4.28,3.32))'
    value = 'if(x<3.66,0,if(x>9.76&x<31.1,2.14,1.66))'
  [../]
...
## Lengths are in meters, prefactors are in mm^2/sec, powers are unitless
    ...
    value = 'if(x<3.66,1e-10,if(x>9.76&x<31.1,1.51e-4,6.1e-3))'
  [../]

  [./Oxide-Activation-Energy-Function-Fe]
    type = ParsedFunction
#    value = 'if(x<3.66,1e-10,if(x>9.76&x<31.1,4.02,3.39))'
    value = 'if(x<3.66,1e-10,if(x>9.76&x<31.1,2.01,1.695))'
  [../]
...
## All length and wetted perimeters should be in meters
    ...
    function = 'if(x<3.66,1704.34,if(x>9.76&x<31.1,1062.99,2.31))'
  [../]
